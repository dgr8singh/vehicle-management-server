...
public class UserDao {
	...
	public User track(String vnum){
		User u=null;
		try {
			PreparedStatement ps=Dao.getCon().prepareStatement("select * from driver where v_number=? ");
			ps.setString(1, vnum);
			ResultSet rs=ps.executeQuery();
			if(rs.next()){
				u=new User();
				...
				u.setUname(rs.getString(2));
				u.setPass(rs.getString(3));
				u.setLon(rs.getString(4));
				u.setLat(rs.getString(5));
				u.setDates(rs.getString(7));
				u.setV_number(vnum);
			}
		} catch (Exception e) {
